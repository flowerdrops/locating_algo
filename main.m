...
WhetherDivide=1;
error_detection=(1<0);      % 1 means open the big error detection protocol
...
%         Toa_e=TR_ToA();
    ...
    if WhetherDivide
        ...
        [x_c,y_c,R_est]=tritangle(nodes(1:3,:),Toa_e,v_guess,S_est(3));
    end
    ...
    error(simInd)=sjj_tmp;
     ...
     %%%%%%%%%%%%%%%%%%%%%%% for debug %%%%%%%%%%%%%%%%%%%%%%%%%%%%
     if (sjj_tmp>1000 & error_detection)
          shenmeguia
     end
%     S
%     S_est
...
error=sqrt(error);      error=real(error)';
