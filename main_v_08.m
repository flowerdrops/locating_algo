...
%%%%%%%%%%% UAV info %%%%%%%%%%
Toa_est_error=16e-3;       % half error
...
%         S=[500,200,-800];
        ...
        for i=1:I
            %%%%%%%%%%%%%% A beacon %%%%%%%%%%%%%%             
            tB(i)=ToA_2nodes( A,B,v_acou );
            %tB(i)=d_ab/v_guess(num_v_guess); 
            tD(i)=ToA_2nodes( A,-D,v_acou );
            %tD(i)=d_ad/v_guess(num_v_guess);
            t1(i)=ToA_2nodes( A,[S(1),S(2),-S(3)],v_acou );
            %t1(i)=d_sa/v_guess(num_v_guess);
            tC(i)=ToA_2nodes( A,C,v_acou );
            %tC(i)=d_ac/v_guess(num_v_guess);
            
            %%%%%%%%%%%%%% B beacon %%%%%%%%%%%%%% 
            tBb(i)=tB(i)+dtb;   
            t2(i)=tBb(i)+ToA_2nodes( B,[S(1),S(2),-S(3)],v_acou );%tBb(i)+d_sb/v_guess(num_v_guess);
            
            %%%%%%%%%%%%%% C beacon %%%%%%%%%%%%%% 
            tCc(i)=tBb(i)+ToA_2nodes( C,B,v_acou );%tBb(i)+d_bc/v_guess(num_v_guess)+dtc;               
            t3(i)=tCc(i)+ToA_2nodes( C,[S(1),S(2),-S(3)],v_acou );%d_sc/v_guess(num_v_guess);
            
            %%%%%%%%%%%%%% D beacon %%%%%%%%%%%%%% 
            tDd(i)=tCc(i)+ToA_2nodes( -D,C,v_acou );
%             tDd(i)=tCc(i)+d_cd/v_guess(num_v_guess)+dtd;   
            t4(i)=tDd(i)+ToA_2nodes( -D,[S(1),S(2),-S(3)],v_acou );
%             t4(i)=tDd(i)+d_sd/v_guess(num_v_guess);

            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%% calculate %%%%%%%%%%%%%%%%%%%%%%%%%%%%
            t=[t1(i),t2(i),t3(i),t4(i)];
            t = est_ToA(t,Toa_est_error);
            dt1=t2(i)-t1(i);    dt2=t3(i)-t1(i);    dt3=t4(i)-t1(i);
            delta_tB=tBb(i)-tB(i);      delta_tC=tCc(i)-tC(i);      delta_tD=tDd(i)-tD(i);
            kk(i,1)=v_guess(num_v_guess)*dt1-v_guess(num_v_guess)*delta_tB-d_ab;  
            kk(i,2)=v_guess(num_v_guess)*dt2-v_guess(num_v_guess)*delta_tC-d_ac;  
            kk(i,3)=v_guess(num_v_guess)*dt3-v_guess(num_v_guess)*delta_tD-d_ad;
        end
